...
static odp_packet_module_t *mod;
...
{
	return ODP_PACKET_API_INVOKE(packet_alloc_multi, pool_hdl, len, pkt, max_num);
}
...
int odp_packet_extend_head(odp_packet_t *pkt, uint32_t len, void **data_ptr,
			   uint32_t *seg_len)
{
	return ODP_PACKET_API_INVOKE(packet_extend_head, pkt, len, data_ptr, seg_len);
}

int odp_packet_trunc_head(odp_packet_t *pkt, uint32_t len, void **data_ptr,
			  uint32_t *seg_len)
{
	return ODP_PACKET_API_INVOKE(packet_trunc_head, pkt, len, data_ptr, seg_len);
}

int odp_packet_extend_tail(odp_packet_t *pkt, uint32_t len, void **data_ptr,
			   uint32_t *seg_len)
{
	return ODP_PACKET_API_INVOKE(packet_extend_tail, pkt, len, data_ptr, seg_len);
}

int odp_packet_trunc_tail(odp_packet_t *pkt, uint32_t len, void **tail_ptr,
			  uint32_t *tailroom)
{
	return ODP_PACKET_API_INVOKE(packet_trunc_tail, pkt, len, tail_ptr, tailroom);
}
...
{
	return ODP_PACKET_API_INVOKE(packet_unshared_len, pkt);
}
...
{
	return ODP_PACKET_API_INVOKE(packet_copy_from_mem, pkt, offset, len, src);
}
